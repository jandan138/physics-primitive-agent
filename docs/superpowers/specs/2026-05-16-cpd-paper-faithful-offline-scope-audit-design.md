...
| `report_schema_tests_and_records` | Keep report schema, tests, registry, and dated records reproducible. | This slice adds RED/GREEN tests, registry metadata, and a dated record path. | `implemented_fixture_scope` | `paper_aligned_boundary` | false | Reproducibility evidence supports the audit record only, not stronger algorithm claims. | Keep records updated for every future gate. |
