...
- [ ] Assert top-level `failure_labels == ["paper_obb_sphere_fit_faithfulness_missing"]`.
- [ ] Assert `next_required_gate == "paper_obb_sphere_fit_faithfulness_audit"`.
- [ ] Assert `paper_polygon_quad_intake_policy_audit` is present in
  `paper_faithfulness.implemented_fixture_scope`.
- [ ] Assert report status boundaries still hold:
  - `status == "partial"`;
  - `paper_faithful_offline_supported is False`;
  - `paper_faithfulness.status == "partial"`.
- [ ] Assert report cases include `paper_quad_face_intake` and `paper_polygon_face_intake` while
  preserving all existing cases.
- [ ] Assert `paper_quad_face_intake["source_mesh"]` records:
...
- [ ] Assert `paper_polygon_face_intake["source_mesh"]` records:
...
- [ ] Assert each new case has `mesh_intake_policy_audit` with:
...
- [ ] Assert each new case's `operator_audit.face_scope` remains `triangle_subfaces_from_source_face`.
- [ ] Assert each new case's `operator_audit.source_face_operator_aggregates` records source face
  ...
  values.
- [ ] Assert each new case's `operator_audit.merged_group.source_faces` and
  `primitive_fit_audit.source_faces` equal the generated triangle ids: `[0, 1]` for the quad and
  `[0, 1, 2]` for the polygon.
- [ ] Assert each new case's `operator_audit.merged_group.generated_triangle_face_ids`,
...
- [ ] Update the CLI JSON test expected case list to this exact order:
  - `paper_single_box`;
...
- [ ] Make the RED assertions observable with separate independent tests:
...
- [ ] Run:
  ...
  visible.
- [ ] Run the new fixture-specific test and confirm it fails for missing `paper_quad_face_intake`
...
- [ ] Add frozen dataclasses `_SourceFaceRemap` and `_SourceFaceIntakeAudit` with typed immutable
  fields:
...
- [ ] Extend `_PaperToyCase` with `source_face_intake_audit: _SourceFaceIntakeAudit | None = None`.
- [ ] Add `_quad_face_intake_mesh()` returning a two-triangle square fan mesh.
- [ ] Add `_polygon_face_intake_mesh()` returning a three-triangle pentagon fan mesh.
- [ ] Add `_source_face_intake_audit_payload(audit)` returning the `mesh_intake_policy_audit`
  fields required by tests.
- [ ] Update `_source_mesh_payload(mesh, source_face_intake_audit=None)` so existing triangle-only
  fixtures are unchanged, and non-triangle fixtures add the source-face policy fields.
- [ ] Update `_operator_audit_payload(mesh, face_groups, source_face_intake_audit=None)` so
  `face_scope` is `triangle_subfaces_from_source_face` only for the new fixtures.
- [ ] Add `_source_face_operator_aggregates(mesh, source_face_intake_audit)` and include aggregate
  source-face rows in `operator_audit` only for the new fixtures.
- [ ] Add generated/source id disambiguation fields to group-level payloads for new fixtures:
  `generated_triangle_face_ids` and `source_face_ids`.
- [ ] Attach `mesh_intake_policy_audit` only when `case.source_face_intake_audit` is present.
- [ ] Add `paper_quad_face_intake` and `paper_polygon_face_intake` to `_paper_toy_cases()`.
- [ ] Replace `polygon_and_quad_face_policy` in `missing_before_paper_faithful` with
  `paper_obb_sphere_fit_faithfulness`.
- [ ] Set `next_required_gate` to `paper_obb_sphere_fit_faithfulness_audit`.
- [ ] Add `paper_polygon_quad_intake_policy_audit` to implemented fixture scope.
...
- [ ] Update current paper-lane wording to say one quad and one five-vertex polygon intake policy
  fixture are recorded.
- [ ] Keep `status: partial` and `paper_faithful_offline_supported: false`.
- [ ] Make `paper_obb_sphere_fit_faithfulness_audit` the next gate.
- [ ] Define the remaining `paper_obb_sphere_fit_faithfulness_missing` label in the gap matrix and
  lane spec.
- [ ] Add an `experiments/registry.yaml` entry:
...
- [ ] Run `git diff --check`.
- [ ] Request multi-agent review for mesh-intake policy schema, docs/claim boundaries, and test
  coverage.
- [ ] Fix Critical/Important review findings before commit.

### Task 5: Commit

- [ ] Commit with message `feat: audit CPD paper polygon quad intake policy`.
- [ ] Push `main`.
