...
configured-runtime preflight, validation, source-resolution, device-resolution, and entry-decision
contracts now
record report-only preflight, default missing-config validation, default missing-source, and default
missing-device rows plus a default no-runtime-entry decision while keeping runtime source/device
resolution, runtime entry, and all real runtime counters false or zero. The current next gate is
