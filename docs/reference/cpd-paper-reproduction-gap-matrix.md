...
1. Add a deterministic offline priority-queue trace over the toy paper lane.
2. Record initial adjacency candidates, minimum-cost pops, accepted/blocked merges, stale-entry
   pruning, updated neighbor candidates, target-count or threshold stop reason, and the candidate
   fit payloads used for each cost.
