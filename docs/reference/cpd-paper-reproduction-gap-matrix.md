...
Newton code. The next implementation slice should keep the same boundary discipline:

1. Implement
   `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_source_resolution_contract`
   only after the bounded configured-runtime validation row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   configured-runtime preflight/validation rows synthetic/report-scoped until source resolution and
   any later reviewed Newton runtime gate explicitly allow broader runtime work.
