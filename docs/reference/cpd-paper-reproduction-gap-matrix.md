...
Batch A and Batch B of the fixture-breadth expansion are now implemented inside the offline report.
The next implementation slice should remain offline only:

1. Implement `paper_fixture_breadth_batch_c` from the fixture-breadth plan.
2. Add only the cost/search/stop breadth fixtures planned for Batch C.
