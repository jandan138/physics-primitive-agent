...
PrimitiveSpecs, generated CollisionPackages, and runtime-admissibility checks at zero. The next
implementation slice should remain offline only:

1. Implement `paper_mapped_subset_primitivespec_runtime_boundary_preflight_contract` after the
   native-fixture serialization/schema-stability contract gate.
2. Define only the report-side boundary for whether a later runtime `PrimitiveSpec` object may be
   proposed, while preserving generated runtime PrimitiveSpec, CollisionPackage,
   runtime-admissibility, Newton, real-USD, and benchmark counts at zero.
