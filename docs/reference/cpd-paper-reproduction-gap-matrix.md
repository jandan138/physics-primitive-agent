...
1. Implement `paper_mapped_subset_newton_shape_runtime_boundary_preflight_contract` after the
   bounded static shape descriptor row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   runtime-admissibility row, shape-mapping preflight row, and static descriptor row
   synthetic/report-scoped until a separate Newton-runtime gate exists.
