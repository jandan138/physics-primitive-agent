...
1. Implement `paper_mapped_subset_newton_shape_mapping_contract` after the bounded static
   shape-mapping preflight row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   runtime-admissibility row, and shape-mapping preflight row synthetic/report-scoped until a
   separate Newton-runtime gate exists.
