...
package-conversion planning table, not a `CollisionPackage`. The next implementation slice should
remain offline only:

1. Implement `paper_mapped_subset_conversion_candidate_matrix` as the next paper-lane planning
   gate before any package generation.
2. Turn the native-family plan into a reviewable candidate matrix, while current paper-only or
   unmapped rows stay offline unless a separate mapping or approximation policy exists.
