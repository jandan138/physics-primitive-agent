...
`newton.ModelBuilder` instantiation, real builder shape calls, model finalization, collision
pipeline calls, and Newton execution at zero. The next implementation slice
...
1. Implement `paper_mapped_subset_newton_shape_runtime_engine_builder_smoke_contract`
   only after the bounded entry decision row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   environment-probe/API-surface records, and engine-builder entry record
