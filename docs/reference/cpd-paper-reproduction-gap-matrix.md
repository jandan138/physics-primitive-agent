...
shape calls, and Newton execution at zero. The
...
1. Implement `paper_mapped_subset_newton_shape_runtime_builder_preflight_contract` after the
   bounded static Newton shape mapping record exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   runtime-admissibility row, shape-mapping preflight row, static descriptor row, and static
   Newton shape runtime-boundary preflight row and repo-local shape mapping record
