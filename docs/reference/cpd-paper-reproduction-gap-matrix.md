...
Newton/Warp runtime, and running no Newton code. The next implementation slice should keep the
same boundary discipline:

1. Implement
   `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_validation_contract`
   only after the bounded configured-runtime preflight row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   configured-runtime preflight row synthetic/report-scoped until configured-runtime validation
   and any later reviewed Newton runtime gate explicitly allow broader runtime work.
