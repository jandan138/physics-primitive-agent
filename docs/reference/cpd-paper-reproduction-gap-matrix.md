...
1. Add enabled component-pair threshold blocking inside the offline priority-queue trace.
2. Record finite threshold settings, disconnected component-pair candidates, accepted/skipped or
   blocked decisions, updated neighbor candidates, target-count or threshold stop reason, and the
   candidate fit payloads used for each cost.
