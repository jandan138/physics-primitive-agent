...
Batch A, Batch B, Batch C, Batch D, and Batch E of the fixture-breadth expansion are now implemented inside the
offline report.
The next implementation slice should remain offline only:

1. Run a `paper_fixture_breadth_completion_review` against the fixture-breadth plan.
2. Decide which scope-audit rows can be reworded and which must remain partial.
