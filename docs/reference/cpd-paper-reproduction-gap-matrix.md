...
finalization, collision pipeline calls, and Newton execution at zero. The next implementation slice
...
1. Implement `paper_mapped_subset_newton_shape_runtime_engine_builder_entry_contract`
   after the bounded API-surface row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   environment-probe/API-surface records
