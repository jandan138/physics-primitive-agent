...
pipeline calls, and Newton execution at zero. The next implementation slice should
keep the same boundary discipline:

1. Implement `paper_mapped_subset_newton_shape_runtime_engine_builder_environment_probe_contract`
   after the bounded engine-builder boundary-preflight checklist exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   builder-preflight call-plan, recording-builder call, and engine-builder boundary-preflight records
