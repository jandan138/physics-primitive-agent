...
1. Implement `paper_package_adapter_unsupported_primitive_policy` as the next paper-lane policy
   gate before package generation.
2. Define how paper-only or unmapped primitive records should be classified before any future
   package-conversion slice, without generating packages or entering Newton runtime execution.
