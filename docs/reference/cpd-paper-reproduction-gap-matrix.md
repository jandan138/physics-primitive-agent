...
| Preprocessing for overlapped or duplicate vertices. The paper discusses vertex deduplication because unclean meshes can change topology and decomposition. | The partial `cpd_paper_offline_report` now includes `paper_duplicate_vertex_preprocessing`, an exact-coordinate duplicate-vertex fixture with before/after vertex counts, first-occurrence vertex remap, duplicate clusters, source-face remap, component counts, and a topology trace over the deduplicated executable mesh. | partial | Paper-shaped for one exact-overlap synthetic fixture only. It is not nonzero-threshold deduplication, approximate spatial hashing, or general mesh cleanup. | Run `paper_faithful_offline_scope_audit` against the full gap matrix and lane spec before stronger paper-lane wording. Broader unclean-mesh policy remains future work. | Not a Newton runtime concern except through the final package produced later. | Do not claim robust unclean-mesh handling, paper-faithful preprocessing, or collision quality from this one fixture. |
| Per-face linear operator `Q`: area-weighted normal outer product plus optional tangent-stability term, summed when face groups merge. | `TriangleMesh.face_operator()` computes an area-weighted triangle normal/tangent matrix. The partial `cpd_paper_offline_report` now records per-face and merged-group operator audit fields for triangle fixtures, plus `paper_quad_face_intake` and `paper_polygon_face_intake` policy fixtures that preserve source-face ids after fan triangulation and record source-face aggregate `Q` matrices as sums of generated triangle subfaces. Operator rows for the duplicate-vertex fixture carry a preprocessing boundary. | partial | Paper-shaped for named fixtures. The quad/polygon path records a conservative fan-triangulation policy for planar, convex, consistently wound toy faces; it is not a general polygon mesh implementation. | Run `paper_faithful_offline_scope_audit` before stronger paper-lane wording. | No Newton mapping; this is algorithm-side data. | This supports "paper-shaped operator/intake audit for named fixtures", not paper-faithful CPD. |
| Primitive vocabulary: oriented bounding box, sphere, capsule, capped cylinder, frustum, and isosceles trapezoidal prism. | Current CPD-like candidates include `box`, `sphere`, `capsule`, `cylinder`, `cone`, `ellipsoid`, and an opt-in `capped_cylinder` proxy. Current Newton mapping supports `box`, `sphere`, `capsule`, `cylinder`, `cone`, and `ellipsoid`. The partial `cpd_paper_offline_report` now includes offline paper-shaped OBB/sphere fit-audit rows, a paper-shaped capsule axis fit-audit row, and offline-only flat-capped-cylinder, frustum, and trapezoidal-prism fit-audit rows for named toy fixtures. | partial | Mixed. `box` and `sphere` are now paper-shaped offline fit-audit rows for named fixtures, but this is still not general paper-faithful primitive fitting. `capsule` has a paper-shaped offline axis audit row with Newton-native kind metadata but no package/runtime execution. `capped_cylinder`, `frustum`, and `trapezoidal_prism` remain offline audit rows only, not runtime support or full paper-faithful coverage. `cone` and `ellipsoid` are Newton-native extensions, not paper primitives. | Run `paper_faithful_offline_scope_audit` before stronger paper-lane wording. | Only the intersection with Newton-native support should run in Newton directly after package conversion and runtime admissibility checks. Paper-only primitives need either an explicit approximation report or must stay offline. | Primitive support is not evidence of better collision quality or full CPD reproduction. |
| Primitive construction from operator eigenvectors and assigned points. The paper fits each primitive by expanding parameters to enclose the assigned point set. | Current fitters use operator axes and point bounds/radial spans, but several are simplified proxies. The partial `cpd_paper_offline_report` audits all six paper primitive names: paper-shaped OBB, paper-shaped sphere, paper-shaped capsule, offline-only flat capped cylinder, offline-only frustum, and offline-only trapezoidal-prism rows. It records paper-specific OBB/sphere clamp `1e-3`, projected bounds, OBB world center, sphere radius from OBB center, axis policy, volume formulas, containment checks, and runtime mapping boundary for each row. | partial | Paper-shaped and fixture-scoped for OBB/sphere/capsule/capped-cylinder/frustum/trapezoidal-prism audit rows. This remains partial because full search breadth, package adaptation, scope audit, and evaluation are not complete. | Run `paper_faithful_offline_scope_audit` before stronger paper-lane wording. | Newton should receive only mapped package primitives after a separate package conversion and runtime-admissibility check. | Candidate audit tables explain current choices only under the current partial offline slice. |
| Collapse cost: merge two primitives by the added volume of the merged primitive over the previous two, with optional primitive type weighting. | Current objective reports include raw and AABB-normalized merge-excess fields. The partial `cpd_paper_offline_report` now records unnormalized `paper_base_cost` separately from `weighted_priority_cost` for one two-face toy fixture, topology priority-queue candidate summaries, one threshold-disabled accepted component-pair candidate, and one finite-threshold blocked component-pair candidate. | partial | Paper-shaped and fixture-scoped. The cost fields match the planned split for toy topology and component-pair edges, but the current threshold-blocking fixture uses a zero raw `paper_base_cost` threshold only. | Run `paper_faithful_offline_scope_audit`; broaden thresholds later only with a separate fixture and record. | Newton should not use cost terms directly; it consumes packages generated after offline selection. | A lower paper-shaped toy cost or blocked toy merge is not a benchmark or quality result. |
| Priority-queue greedy collapse over adjacent face groups. The paper initializes adjacent face-pair candidates and repeatedly collapses the minimum-cost candidate. | The partial `cpd_paper_offline_report` now includes a topology-only priority-queue trace over `paper_three_face_chain`, a threshold-disabled component-pair insertion trace over `paper_disconnected_components`, a finite-threshold blocked component-pair trace over `paper_component_pair_threshold_blocked`, and a deduplicated-topology trace over `paper_duplicate_vertex_preprocessing`, with deterministic queue keys, accepted/blocked decisions, stale-prune fields where applicable, component-pair insertion metadata, final active groups, and stop reasons. | partial | Paper-shaped for named toy fixtures. It is not full paper decomposition because search traces remain toy-scoped and the scope audit has not accepted stronger wording. | Run `paper_faithful_offline_scope_audit` next. | Newton package probes should wait until the offline paper lane produces a changed package. | Toy search traces do not prove merge-policy superiority. |
| Target primitive count `N` and optional excess-volume threshold. The paper stops when no collapses remain or the target count is reached; thresholding can block high-excess merges. | Current configs have max primitive limits and some threshold-style component-merge gates. The partial `cpd_paper_offline_report` now records `target_primitive_count`, disabled or zero finite `excess_volume_threshold`, accepted and blocked component-pair counts, skipped component-pair count `0`, and target-count or threshold-blocked stop reasons for named toy traces. | partial | Partial and fixture-scoped. The finite threshold case uses `threshold_metric: paper_base_cost` and a zero threshold to make the decision deterministic. | Run `paper_faithful_offline_scope_audit`; broaden threshold policies later only with a separate fixture and record. | Newton should inherit the selected package only after the offline lane records final count and stop reason. | A stopped run is not success unless the report states which target and gates passed. |
| Pairwise component merging when disconnected topology prevents reaching the target. | Current `virtual_pairwise` component-merge diagnostics and synthetic fixtures cover a restricted CPD-like version. The partial `cpd_paper_offline_report` now inserts and accepts one threshold-disabled `component_pair` queue event for `paper_disconnected_components` and inserts then blocks one finite-threshold `component_pair` event for `paper_component_pair_threshold_blocked`, with attempted count `1` and skipped count `0`. | partial | Paper-shaped but not paper-faithful. It records accepted and blocked toy component-pair behavior, with cap-skipped pairs out of scope for this all-pairs fixture. | Run `paper_faithful_offline_scope_audit`; add capped skipped-pair fixtures later only if needed. | Runtime should only see the resulting package after a separate package-generation slice. | Component merging evidence is diagnostic accounting, not broad asset evidence. |
| Postprocessing to remove primitives enclosed by other primitives. | The partial `cpd_paper_offline_report` now includes `paper_nested_primitive`, an explicit two-OBB fixture that records one identity-axis inner primitive enclosed by one larger identity-axis outer primitive. It records before/after counts, cull reason, corner containment status, and package/Newton/real-USD/benchmark false triggers. | partial | Paper-shaped fixture accounting only. The input primitives are explicit audit rows, not generated by the full paper search, and the containment check is a shared-identity-axis canary, not a general primitive containment library. | General postprocess breadth can expand later only with separate fixtures and records. | Newton should consume postprocessed packages only after package generation and runtime admissibility are separately recorded. | This supports one offline postprocess cull audit, not redundant primitive removal quality, runtime support, or full CPD reproduction. |
...
1. Run `paper_faithful_offline_scope_audit` against this gap matrix and the offline lane spec.
2. Decide which named fixture mechanics are paper-shaped enough for bounded offline wording and
   which still need fixture expansion.
