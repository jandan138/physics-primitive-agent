...
topology, threshold, and component-pair traces. The next implementation slice should remain
offline only:

1. Implement `paper_generalization_batch_d_postprocess_policy` as the next offline generalization
   slice after the search-engine matrix.
2. Generalize postprocess policy accounting for enclosed-primitive culling boundaries without
   entering package generation or Newton runtime execution.
