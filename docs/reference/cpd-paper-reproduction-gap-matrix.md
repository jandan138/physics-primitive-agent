...
`CollisionPackage`. The next implementation slice should remain offline only:

1. Implement `paper_mapped_subset_primitivespec_dry_run_contract` as the next paper-lane contract
...
2. Define report-only PrimitiveSpec dry-run requirements and zero-candidate behavior while current
   paper-only or unmapped rows stay offline unless a separate mapping or approximation policy
   exists.
