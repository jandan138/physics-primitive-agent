...
1. Implement `paper_mapped_subset_adapter_preflight_contract` as the next paper-lane contract gate
   before any package generation.
2. Define adapter preflight requirements and no-op behavior while current paper-only or unmapped
   rows stay offline unless a separate mapping or approximation policy exists.
