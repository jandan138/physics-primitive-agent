...
| Frustum | missing | no direct runtime primitive | Keep offline unless an explicit approximation policy exists. Newton `cone` is not general frustum support. |
| Isosceles trapezoidal prism | reported as missing `trapezoidal_prism` in current gap accounting | no direct runtime primitive | Keep offline unless converted through a recorded convex-hull or other adapter. |
...
| Per-face linear operator `Q`: area-weighted normal outer product plus optional tangent-stability term, summed when face groups merge. | `TriangleMesh.face_operator()` computes an area-weighted triangle normal/tangent matrix. Candidate axes are derived from the summed operator. | partial | Paper-shaped for triangles, but not full paper handling for polygons, tangent policy, or recorded operator audits. | Add an operator audit report with per-face area, normal, tangent, epsilon, eigenvalues, eigenvectors, and degeneracy labels. | No Newton mapping; this is algorithm-side data. | This supports "paper-shaped operator plumbing", not paper-faithful CPD. |
| Primitive vocabulary: oriented bounding box, sphere, capsule, capped cylinder, frustum, and isosceles trapezoidal prism. | Current CPD-like candidates include `box`, `sphere`, `capsule`, `cylinder`, `cone`, `ellipsoid`, and an opt-in `capped_cylinder` proxy. Current Newton mapping supports `box`, `sphere`, `capsule`, `cylinder`, `cone`, and `ellipsoid`. | partial | Mixed. `box`/`sphere`/`capsule` overlap with paper intent. Current `capped_cylinder` is a proxy package kind, not the paper fit and not Newton-mapped. `cone` and `ellipsoid` are Newton-native extensions, not paper primitives. `frustum` and `trapezoidal_prism` are missing. | Implement offline paper primitive classes for all six paper primitives, including paper-style capped cylinder, frustum, and isosceles trapezoidal prism fitting and volume. | Only the intersection with Newton-native support should run in Newton directly after package conversion and runtime admissibility checks. Paper-only primitives need either an explicit approximation report or must stay offline. | Primitive support is not evidence of better collision quality or full CPD reproduction. |
| Primitive construction from operator eigenvectors and assigned points. The paper fits each primitive by expanding parameters to enclose the assigned point set. | Current fitters use operator axes and point bounds/radial spans, but several are simplified proxies. | surrogate | Surrogate. It produces useful diagnostic candidates but does not implement the paper's full construction details. | Add paper-specific fitting functions, containment checks, and exact per-primitive volume fields. Compare them on tiny synthetic fixtures before real assets. | Newton should receive only mapped package primitives after the offline report marks them as runtime-admissible. | Candidate audit tables explain current choices only under the current surrogate. |
| Collapse cost: merge two primitives by the added volume of the merged primitive over the previous two, with optional primitive type weighting. | Current objective reports include raw and AABB-normalized merge-excess fields, plus weighted-volume accounting. Some toy merge decisions use the normalized surrogate. | partial | Paper-aligned surrogate. The category is aligned, but the current cost units, normalization, and fitters are not a paper-faithful implementation. | Implement a paper cost report with unnormalized paper collapse cost, weighted volume `k(p)V(p)`, configured paper weights, and optional threshold fields. | Newton should not use cost terms directly; it consumes packages generated after offline selection. | A lower surrogate cost is not a benchmark or quality result. |
