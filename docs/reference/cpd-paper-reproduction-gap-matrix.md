...
zero. The next implementation slice should keep the same boundary discipline:

1. Implement `paper_mapped_subset_collision_package_generation_contract` after the preflight
   candidate exists.
2. Keep the constructed runtime `PrimitiveSpec` object and preflight candidate
   synthetic/report-scoped until a separate runtime-admissibility and Newton-runtime gate exists.
...
4. Keep Newton, bed, Franka, runtime admissibility, and benchmark work out of scope until a
   separate dated mapped-subset runtime record says what may be mapped and checked.
