...
eligible current PrimitiveSpec candidate sources, and keeps runtime/package checks at zero. The next
implementation slice should remain offline only:

1. Implement `paper_mapped_subset_native_current_fixture_contract` as the next paper-lane contract
   gate before any real runtime `PrimitiveSpec` or package generation.
2. Add or select a deterministic synthetic current row in the Newton-native mapped subset, while
   preserving candidate-source zero-runtime behavior unless that row is explicitly traceable,
   native-family mapped, and admissible under the offline contract.
...
4. Keep Newton, bed, Franka, package generation, and benchmark work out of scope until a separate
   dated mapped-subset and package-generation record says what may be mapped and checked.
