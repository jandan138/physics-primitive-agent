...
engine shape objects, and Newton execution at zero. The
next implementation slice should
keep the same boundary discipline:

1. Implement `paper_mapped_subset_newton_shape_runtime_builder_construction_contract` after the
   bounded static builder-preflight call-plan record exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   builder-preflight call-plan record
