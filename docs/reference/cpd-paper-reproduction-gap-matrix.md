...
validation contract, not real `PrimitiveSpec` generation and not a `CollisionPackage`. The next
implementation slice should remain offline only:

1. Implement `paper_mapped_subset_primitivespec_generation_preflight_contract` as the next
   paper-lane contract gate before any real `PrimitiveSpec` or package generation.
2. Preserve report-only PrimitiveSpec validation requirements and zero-candidate behavior while
