...
dry-run contract, not real `PrimitiveSpec` generation and not a `CollisionPackage`. The next
implementation slice should remain offline only:

1. Implement `paper_mapped_subset_primitivespec_validation_contract` as the next paper-lane
   contract gate before any real `PrimitiveSpec` or package generation.
2. Validate report-only PrimitiveSpec dry-run requirements and zero-candidate behavior while
