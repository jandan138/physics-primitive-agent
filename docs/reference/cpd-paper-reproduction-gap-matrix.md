...
implementation slice should keep the same boundary discipline:

1. Implement `paper_mapped_subset_primitivespec_runtime_construction_contract` only if it can keep
   the current `paper_single_box` lineage, schema, and nonclaim fields explicit.
2. If a runtime `PrimitiveSpec` object is proposed, keep it offline/report-scoped until a separate
   package-generation and Newton-runtime gate exists; do not silently convert it into a
   `CollisionPackage`.
