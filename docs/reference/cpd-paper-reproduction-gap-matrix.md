...
1. Implement `paper_mapped_subset_newton_shape_runtime_construction_contract` after the bounded
   static shape runtime-boundary preflight row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   Newton shape runtime-boundary preflight row synthetic/report-scoped until a separate
