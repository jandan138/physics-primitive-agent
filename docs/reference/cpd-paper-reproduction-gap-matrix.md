...
existing deterministic postprocess audit fixtures. The next implementation slice should remain
offline only:

1. Implement `paper_generalization_batch_e_package_boundary_readiness` as the next offline
   package-boundary readiness review after the postprocess-policy matrix.
2. Review whether the offline lane has enough changed decomposition output to justify package
   generation later, without entering Newton runtime execution in this slice.
...
4. Keep Newton, bed, Franka, package generation, and benchmark work out of scope until a separate
   dated package-boundary record says what may be mapped and checked.
