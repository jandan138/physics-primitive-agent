...
Batch A, Batch B, Batch C, Batch D, Batch E, and the command-only synthetic
fixture-breadth completion review are now implemented inside the offline report. The next
implementation slice should remain offline only:

1. Define `paper_faithful_offline_generalization_plan` as a planning-only gate for broadening the
   offline algorithm beyond named toy fixtures.
2. Keep the report partial and keep `paper_faithful_offline_supported: false` until a later dated
   record proves stronger offline mechanics.
