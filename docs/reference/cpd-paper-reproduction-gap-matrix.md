...
1. Implement `paper_mapped_subset_primitivespec_native_fixture_generation_contract` as the next
   paper-lane contract gate before any real runtime `PrimitiveSpec` or package generation.
2. Generate only the report-only native fixture PrimitiveSpec row for the deterministic
   `paper_single_box` OBB/box source, while preserving generated runtime PrimitiveSpec,
