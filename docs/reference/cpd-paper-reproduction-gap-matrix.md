...
matrix before package conversion. The next implementation slice should remain offline only:

1. Implement `paper_offline_changed_decomposition_output_contract` as the next paper-lane contract
   before package generation.
2. Define a durable changed-decomposition output that a later package adapter can consume, without
   entering Newton runtime execution in this slice.
