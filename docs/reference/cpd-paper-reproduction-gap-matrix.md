...
source-policy matrix for deterministic synthetic meshes. The next implementation slice should
remain offline only:

1. Implement `paper_generalization_batch_b_primitive_fit_engine` as the next offline
   generalization slice after the source-policy matrix.
2. Generalize primitive-fit engine accounting for the paper primitive set beyond named toy
