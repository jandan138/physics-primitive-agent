...
checks at zero. The next implementation slice should keep the same boundary discipline:

1. Implement `paper_mapped_subset_collision_package_generation_preflight_contract` before any
   package-generation attempt.
2. Keep the constructed runtime `PrimitiveSpec` object offline/report-scoped until a separate
