...
offline changed-decomposition output contract, not a `CollisionPackage`. The next implementation
slice should remain offline only:

1. Implement `paper_package_adapter_contract` as the next paper-lane adapter contract before
   package generation.
2. Define how a later adapter may consume the offline changed-decomposition output, without
   generating packages or entering Newton runtime execution in this slice.
...
4. Keep Newton, bed, Franka, package generation, and benchmark work out of scope until a separate
   dated package-boundary/package-generation record says what may be mapped and checked.
