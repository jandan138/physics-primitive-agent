...
check. The next implementation slice should keep the same boundary discipline:

1. Implement `paper_mapped_subset_runtime_admissibility_contract` after the bounded preflight row
   exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   `CollisionPackage.to_dict()` artifact, and runtime-admissibility preflight row
