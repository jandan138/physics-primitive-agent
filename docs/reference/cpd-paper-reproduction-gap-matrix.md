...
box-schema check without Newton mapping or Newton execution. The next implementation slice should
keep the same boundary discipline:

1. Implement `paper_mapped_subset_newton_shape_mapping_preflight_contract` after the bounded
   static runtime-admissibility row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   runtime-admissibility row synthetic/report-scoped until a separate Newton-runtime gate exists.
