...
1. Implement
   ...
   only after the bounded configured-runtime entry-decision row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   configured-runtime preflight/validation/source-resolution/device-resolution rows
   synthetic/report-scoped until entry decision and any later reviewed Newton runtime gate
