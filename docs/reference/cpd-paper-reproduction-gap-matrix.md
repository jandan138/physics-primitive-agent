...
1. Implement
   ...
   only after the bounded configured-runtime source-resolution row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   configured-runtime preflight/validation/source-resolution rows synthetic/report-scoped until
   device resolution and any later reviewed Newton runtime gate explicitly allow broader runtime
   work.
