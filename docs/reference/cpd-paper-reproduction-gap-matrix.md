...
does not add Newton, real-USD, benchmark, or collision-quality evidence. The next implementation
slice should keep the same boundary discipline:

1. Implement `paper_mapped_subset_runtime_admissibility_preflight_contract` after the bounded
   package-shaped artifact exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, and generated
   `CollisionPackage.to_dict()` artifact synthetic/report-scoped until a separate
   runtime-admissibility and Newton-runtime gate exists.
