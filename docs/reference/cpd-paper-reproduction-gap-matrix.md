...
shape's expected dimension schema. Passing those checks is runtime-admissibility/schema preflight;
it is not runtime compatibility and is not paper faithfulness.
...
while keeping real runtime imports and Newton execution at zero. The Newton engine-builder
API-surface contract now records default no-config source-AST API-surface status while keeping
real runtime imports, `newton.ModelBuilder` instantiation, real builder shape calls, model
finalization, collision pipeline calls, and Newton execution at zero. The engine-builder entry
contract now records a default no-runtime-entry decision while keeping real runtime imports,
`newton.ModelBuilder` instantiation, real builder shape calls, model finalization, collision
pipeline calls, and Newton execution at zero. The engine-builder smoke contract now records a
default skipped-smoke decision while keeping runtime-smoke attempts and Newton execution at zero.
The engine-builder runtime-execution contract now records a default skipped-runtime-execution
decision while keeping runtime-execution attempts and Newton execution at zero. The engine-builder
runtime-lane review contract now records a report-only claim-boundary review for that skipped
runtime-execution row, keeps runtime compatibility unvalidated, and keeps all real runtime counters
at zero. The engine-builder configured-runtime design contract now records required runtime inputs
for that same lineage while keeping runtime config validation false and all real runtime counters
at zero. The engine-builder configured-runtime preflight contract now records the bounded preflight
decision while still reading no config, resolving no runtime source/device, importing no
Newton/Warp runtime, and running no Newton code. The engine-builder configured-runtime validation
contract now records the default missing-config validation result while reading no config file or
environment, resolving no runtime source/device, importing no Newton/Warp runtime, and running no
Newton code. The engine-builder configured-runtime source-resolution contract now records the
default missing `newton.source_dir` source-resolution result while performing no filesystem probe,
resolving no runtime source/device, importing no Newton/Warp runtime, and running no Newton code.
The engine-builder configured-runtime device-resolution contract now records the default missing
`newton_diagnostic.device` result while creating no runtime device object, resolving no runtime
source/device, importing no Newton/Warp runtime, and running no Newton code.
The engine-builder configured-runtime entry-decision contract now records the default
no-runtime-entry decision while keeping runtime entry allowed/attempted/passed false, importing no
Newton/Warp runtime, and running no Newton code.
The engine-builder configured-runtime smoke contract now records the default skipped-smoke decision
while keeping runtime smoke allowed/attempted/passed false, importing no Newton/Warp runtime, and
running no Newton code.
The engine-builder configured-runtime execution contract now records the default skipped-execution
decision while keeping runtime execution allowed/attempted/passed false, importing no Newton/Warp
runtime, and running no Newton code.
The engine-builder configured-runtime lane-review contract now records the report-only
claim-boundary review for that skipped-execution row while keeping real runtime evidence, runtime
compatibility, and configured-runtime run allowed/attempted/passed false.
The next implementation slice should keep the same boundary discipline:

1. Implement
   `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_run_contract`
   only after the bounded configured-runtime lane-review row exists.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, generated
   ...
   builder-preflight call-plan, recording-builder call, engine-builder boundary-preflight,
   environment-probe/API-surface records, engine-builder entry record, skipped-smoke record,
   skipped-runtime-execution record, runtime-lane review row, configured-runtime design row, and
   configured-runtime preflight/validation/source-resolution/device-resolution/entry-decision/smoke/execution
   rows synthetic/report-scoped until a later reviewed Newton runtime gate
   explicitly allow broader runtime work.
