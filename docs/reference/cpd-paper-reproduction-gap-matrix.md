...
fixture-breadth completion review are now implemented inside the offline report. The
command-only generalization planning table is now also implemented. The next implementation slice
should remain offline only:

1. Implement `paper_generalization_batch_a_source_policy` as the first offline generalization
   slice after the planning table.
2. Broaden source mesh, preprocessing, source-face intake, and operator policy beyond named toy
   fixtures without entering package generation or Newton runtime execution.
