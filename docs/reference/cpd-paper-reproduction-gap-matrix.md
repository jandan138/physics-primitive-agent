...
`CollisionPackage`. The next
implementation slice should remain offline only:

1. Implement `paper_mapped_subset_primitivespec_candidate_source_contract` as the next paper-lane
   contract gate before any real runtime `PrimitiveSpec` or package generation.
2. Preserve report-only PrimitiveSpec generation template requirements and zero-candidate behavior
