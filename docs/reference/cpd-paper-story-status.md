...
66. `paper_mapped_subset_newton_shape_runtime_engine_builder_entry_contract` is now implemented
    as a single-fixture report-only engine-builder entry decision, not a real Newton import, not a
    `newton.ModelBuilder` instantiation, not a real builder call, not model finalization, not a
    collision pipeline, and not Newton execution. It consumes the API-surface row for the same
    synthetic `paper_single_box` lineage, records `entry_decision: defer_real_runtime_entry`,
...
68. `paper_mapped_subset_newton_shape_runtime_engine_builder_runtime_execution_contract` is now
    implemented as a single-fixture report-only skipped-runtime-execution decision, not a real
    Newton import, not a `newton.ModelBuilder` instantiation, not a real builder call, not model
    finalization, not a collision pipeline, and not Newton execution. It consumes the smoke row
    for the same synthetic `paper_single_box` lineage, records
    `runtime_execution_decision: skip_real_runtime_execution`, keeps runtime-execution attempts
    and all real runtime counters at zero, keeps the report partial, and points next to the
    ...
    runtime counters at zero, and at that stage pointed next to
    `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_validation_contract`.
    The configured-runtime validation contract is now closed as a report-only missing-config
    validation record, reads no config file or environment, keeps runtime source/device resolution
    false, keeps all real runtime counters at zero, and at that stage pointed next to
    ...
    The configured-runtime entry-decision contract is now closed as a report-only no-runtime-entry
    decision, keeps runtime entry allowed/attempted/passed false, keeps all real runtime counters at
    zero, and points next to
    `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_smoke_contract`.
69. `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_smoke_contract` is
    now implemented as a single-fixture report-only skipped-smoke decision, not a real Newton import,
    not a `newton.ModelBuilder` instantiation, not a real builder call, not model finalization, not a
    collision pipeline, and not Newton execution. It consumes the configured-runtime entry-decision
    row for the same synthetic `paper_single_box` lineage, records
    `skip_real_runtime_smoke_missing_configured_runtime_entry`, keeps runtime-smoke attempts and all
    real runtime counters at zero, keeps the report partial, and points next to
    `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_execution_contract`.
70. Records and configs can preserve exactly what was run.
...
contract now also exists as a report-only skipped-smoke decision for the same lineage. The
engine-builder runtime-execution contract now also exists as a report-only
...
as a report-only preflight record with runtime config validation still false. The engine-builder
...
resolution still false. The engine-builder configured-runtime entry-decision contract now also
...
runtime smoke still false. The next code slice is
`paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_execution_contract`.
...
-> historical mapped collision package using previously recorded diagnostic primitive mappings
...
-> next: paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_execution_contract
...
1. Implement
   `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_execution_contract`
   only after the report-only configured-runtime smoke row exists. That future execution gate must
   still start from the single synthetic `paper_single_box` lineage and keep real runtime execution
   blocked unless configured source/device resolution and runtime entry/smoke have passed.
2. Keep the constructed runtime `PrimitiveSpec` object, preflight candidate, synthetic
   ...
   configured-runtime design/preflight/validation/source-resolution/device-resolution/entry-decision/smoke rows for the
...
4. Keep `paper_faithful_offline`, full CPD reproduction, Newton support/execution, real USD,
   ...
   unsupported; the next gate may only review the bounded runtime lane for this one synthetic
   lineage while preserving the no real import, no `newton.ModelBuilder` instantiation, no real
   builder shape call, no model finalization, no collision pipeline execution, no real USD, no
   benchmark, and no quality boundary changes until a later dated gate explicitly records them.
