...
70. `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_execution_contract`
    is now implemented as a single-fixture report-only skipped-execution decision, not a real
    Newton import, not a `newton.ModelBuilder` instantiation, not a real builder call, not model
    finalization, not a collision pipeline, and not Newton execution. It consumes the
    configured-runtime smoke row for the same synthetic `paper_single_box` lineage, records
