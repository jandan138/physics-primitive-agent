...
real USD, running benchmarks, or measuring collision quality. The configured-runtime
  preflight/validation/source-resolution/device-resolution/entry-decision/smoke gates are now also
  closed as report-only missing-config, missing-source, missing-device, no-runtime-entry, and
  skipped-smoke records. The
...
deployment/safety evidence, not full-CPD evidence, not `paper_faithful_offline` evidence, and not
  paper primitive vocabulary coverage. The next step is a future configured-runtime execution
...
report-only skipped-smoke, skipped-runtime-execution, runtime-lane review, and configured-runtime
  design/preflight/validation/source-resolution/device-resolution/entry-decision/smoke contracts:
  `paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_execution_contract`. The serialization contract
...
- [CPD paper faithful offline generalization plan](reference/cpd-paper-faithful-offline-lane-spec.md):
  ...
  design/preflight/validation/source-resolution/device-resolution/entry-decision/smoke decisions are now
...
- [2026-05-20 CPD Paper Newton Engine-Builder Configured-Runtime Smoke Contract](records/2026-05-20-cpd-paper-newton-engine-builder-configured-runtime-smoke-contract.md):
  ...
  runtime counters at zero, and advances the current next gate to configured-runtime execution.
...
- `npc-compile --run-cpd-paper-offline-report`: command-only partial offline paper-lane audit over
  ...
  configured-runtime design/preflight/validation/source-resolution/device-resolution/entry-decision/smoke
  contract sequence that closes only the corresponding configured-runtime report-only contracts as
  one single-fixture missing-config, missing-source, missing-device, no-runtime-entry,
  skipped-smoke path.
  It keeps
  scope-audit table
  with `decision: remain_partial`, reports
  `next_required_gate: paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_execution_contract`,
  keeps
