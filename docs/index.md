...
creates no collision pipeline, and runs zero Newton runtime code. The report now also closes the
single-fixture engine-builder entry contract as a report-only default no-runtime-entry decision.
It records `entry_decision: defer_real_runtime_entry`, allows zero runtime entry attempts, imports
no real Newton/Warp runtime, instantiates no `newton.ModelBuilder`, makes zero real builder shape
calls, finalizes no model, creates no collision pipeline, and runs zero Newton runtime code. The
engine-builder smoke contract is implemented only as a report-only skipped-smoke decision for the
same default no-runtime-entry path. The report now also closes the single-fixture engine-builder
runtime-execution contract as a report-only skipped-runtime-execution decision. It records
...
creates no collision pipeline, and runs zero Newton runtime code. The report-only runtime-lane
review contract records
`runtime_lane_review_decision: keep_real_runtime_execution_blocked`, preserves the claim boundary,
and does not treat skipped runtime execution as runtime compatibility. The configured-runtime
design contract is now closed as a report-only runtime input design record for that same single
synthetic lineage. It defines required runtime-source, runtime-device, entry-decision,
...
real USD, running benchmarks, or measuring collision quality. The configured-runtime
  preflight/validation/source-resolution/device-resolution/entry-decision/smoke/execution/lane-review
  gates are now also closed as report-only missing-config, missing-source, missing-device,
  no-runtime-entry, skipped-smoke, skipped-execution, and claim-boundary review records. The
...
deployment/safety evidence, not full-CPD evidence, not `paper_faithful_offline` evidence, and not
  paper primitive vocabulary coverage. The next step is a future configured-runtime run contract
...
contracts:
`paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_run_contract`. The serialization contract
...
- [CPD paper faithful offline generalization plan](reference/cpd-paper-faithful-offline-lane-spec.md):
  ...
  design/preflight/validation/source-resolution/device-resolution/entry-decision/smoke/execution/lane-review decisions are now
...
- [CPD paper Newton engine-builder gate consolidation record](records/2026-05-19-cpd-paper-newton-engine-builder-gate-consolidation.md):
  ...
  import-boundary-preflight/import-contract gates from the plan, and defined the consolidated
  engine-builder entry contract as the post-API-surface gate.
- [2026-05-20 CPD Paper Newton Engine-Builder Configured-Runtime Design Contract](records/2026-05-20-cpd-paper-newton-engine-builder-configured-runtime-design-contract.md):
...
- [2026-05-20 CPD Paper Newton Engine-Builder Configured-Runtime Entry-Decision Contract](records/2026-05-20-cpd-paper-newton-engine-builder-configured-runtime-entry-decision-contract.md):
  ...
  keeps runtime entry false, keeps all real runtime counters at zero, and at that stage advanced the
  next gate to configured-runtime smoke.
- [2026-05-20 CPD Paper Newton Engine-Builder Configured-Runtime Smoke Contract](records/2026-05-20-cpd-paper-newton-engine-builder-configured-runtime-smoke-contract.md):
  dated implementation record for the single-fixture report-only configured-runtime smoke contract.
  It consumes the entry-decision row, records a skipped real-runtime-smoke decision, keeps all real
  runtime counters at zero, and at that stage advanced the next gate to configured-runtime
  execution.
- [2026-05-20 CPD Paper Newton Engine-Builder Configured-Runtime Execution Contract](records/2026-05-20-cpd-paper-newton-engine-builder-configured-runtime-execution-contract.md):
  dated implementation record for the single-fixture report-only configured-runtime execution
  contract. It consumes the skipped-smoke row, records
  `skip_real_runtime_execution_configured_runtime_smoke_not_allowed`, keeps all real runtime
  counters at zero, and at that stage advanced the next gate to configured-runtime lane review.
- [2026-05-20 CPD Paper Newton Engine-Builder Configured-Runtime Lane-Review Contract](records/2026-05-20-cpd-paper-newton-engine-builder-configured-runtime-lane-review-contract.md):
  dated implementation record for the single-fixture report-only configured-runtime lane-review
  contract. It consumes the skipped-execution row, keeps real runtime evidence and runtime
  compatibility false, keeps all real runtime counters at zero, and advances the current next gate
  to configured-runtime run.
...
- `npc-compile --run-cpd-paper-offline-report`: command-only partial offline paper-lane audit over
  ...
  execution, plus an engine-builder entry contract that closes only
  `paper_mapped_subset_newton_shape_runtime_engine_builder_entry_contract` as one report-only
  default `defer_real_runtime_entry` decision with zero real runtime-entry and Newton engine shape
  object counters, plus an engine-builder smoke contract that closes only
  `paper_mapped_subset_newton_shape_runtime_engine_builder_smoke_contract` as one report-only
  `skip_real_runtime_smoke` decision with zero runtime-smoke attempts and zero Newton engine shape
  objects, plus an engine-builder runtime-execution contract that closes only
  `paper_mapped_subset_newton_shape_runtime_engine_builder_runtime_execution_contract` as one
  report-only `skip_real_runtime_execution` decision with zero runtime-execution attempts and
  ...
  contract sequence that closes only the corresponding configured-runtime report-only
  contracts as one single-fixture missing-config, missing-source, missing-device, no-runtime-entry,
  skipped-smoke, skipped-execution, and claim-boundary review path.
  It keeps
  scope-audit table
  with `decision: remain_partial`, reports
  `next_required_gate: paper_mapped_subset_newton_shape_runtime_engine_builder_configured_runtime_run_contract`,
  keeps
