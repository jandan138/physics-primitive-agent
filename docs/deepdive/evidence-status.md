...
- The current executable surface can run `cpd_paper_offline_report`, a command-only partial
  ...
  `paper_fixture_breadth_expansion_plan`. The current gate is now updated below by Batch B. This is
...
- The report also includes the Batch A fixture-breadth source/preprocess/intake/operator slice
  ...
  offline slice; Batch B below advances the current report gate to `paper_fixture_breadth_batch_c`.
...
- The current executable surface can run the Batch B fixture-breadth primitive-fit slice inside
  ...
  The report remains partial and points the next gate to `paper_fixture_breadth_batch_c`; this is
