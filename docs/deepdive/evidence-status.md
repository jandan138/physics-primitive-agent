...
- The current executable surface can also run
  ...
  source/device, imports no Newton/Warp runtime, and runs no Newton code. The bounded
  configured-runtime device-resolution slice then records the default missing
  `newton_diagnostic.device` result, creates no runtime device object, resolves no runtime source or
  device, imports no Newton/Warp runtime, and runs no Newton code. The bounded
  configured-runtime entry-decision slice then records the default no-runtime-entry decision,
  attempts no runtime entry, imports no Newton/Warp runtime, and runs no Newton code. The bounded
  configured-runtime smoke slice then records
...
- The [2026-05-21 Franka native opt-in support-threshold probe](../records/2026-05-21-franka-native-opt-in-support-threshold-probe.md)
  adds a separate capped Franka opt-in support-admissibility slice. The default capped Franka
  native lane remains `32` boxes; the support-threshold opt-in lane lowers only configured
  `cylinder` extension thresholds to `2` source faces and `4` unique points, selects `29` boxes
  plus `3` cylinders, fully maps, and passes contact-gated drop/settle plus sphere-rain in the
  clean Newton environment. This is controlled support-threshold diagnostic evidence only; it
  does not change default support-aware lanes, calibrate support thresholds, validate cylinder
  quality, or prove whole-robot Franka collider quality.
- The [2026-05-21 bed/Franka guarded support-threshold probe](../records/2026-05-21-bed-franka-guarded-support-threshold-probe.md)
  composes the opt-in large-flat-cylinder selection guard with the opt-in cylinder support
  threshold relaxation in one two-role config and uses no score multiplier. In that recorded
  config, capped bed remains `32` boxes and capped Franka native-opt-in selects `29` boxes plus
...
- The 2026-05-21 Franka support-threshold opt-in record is a separate follow-up algorithm slice:
  it admits the three default support-blocked raw-cost Franka cylinder candidates in one opt-in
  lane and reaches the recorded task smokes with `29` boxes plus `3` cylinders. This is
  support-admissibility diagnostic evidence only, not a default-policy, calibrated-threshold, or
  collision-quality result.
- The 2026-05-21 bed/Franka guarded support-threshold opt-in record composes those two opt-in
  controls in one two-role config without a score multiplier. Guarded bed stays at `32` boxes;
