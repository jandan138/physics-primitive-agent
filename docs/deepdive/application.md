...
- a scoped Phase 0 follow-up run records bounding-primitive, CPD-style, CoACD convex-mesh, and
  ...
  assets and records probe failures on bowl/cup/tray as explicit diagnostic rejections rather
  than dependency gaps; the same report records one Franka link-aware package with zero cross-link
  merges, one Franka USD articulation smoke, and one generated-package Franka task smoke that
  consumes all 12 generated package primitives.
...
- keep recorded V-HACD probe failures visible as diagnostic failures, with bounded sensitivity
  reruns only if needed for reviewer-facing baseline context;
