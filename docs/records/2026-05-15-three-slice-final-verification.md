...
Merged to `master` and verified after merge.
...
- Branch: `master`
- Feature commit merged by fast-forward: `8eee917`
...
- `PYTHONPATH=src /cpfs/user/zhuzihou/conda-managed/envs/physics-primitive-newton-py310/bin/python -m primitive_collision_compiler.cli --config configs/experiments/franka_cpd_like_smoke.yaml --check-assets`: exit 0.
...
- Sphere-rain: `smoke_passed`, 32/32 primitives mapped, 9 probe spheres, max raw package-probe
  ...
  failure labels.
- Franka asset open smoke: `smoke_passed`, hash matched, default prim `/panda`, 369 prims,
  meters per unit `1.0`, Z-up.
...
Use this as the baseline for the next CPD reproduction slice: paper-aligned objective terms,
offline inspection reports, and only then a stricter Newton task beyond smoke checks.
